...
export function requirePortalServerScope(
  ...
  if (!isValidToolName(scope.serverId)) throw new Error("Portal 服务器 ID 无效。");
  for (const name of scope.tools ?? []) {
    if (!isValidToolName(name)) throw new Error("MCP 工具名称无效。");
    if (isPortalNativeTool(name)) {
      throw new Error(`不能授权 Portal 管理工具“${name}”。`);
    }
    if (!isPortalToolGrantable(name, scope.serverId)) {
      throw new Error(`工具“${name}”不属于 Portal 服务器“${scope.serverId}”。`);
    }
  }
}
