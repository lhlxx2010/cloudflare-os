...
export class McpSessionBase extends RpcTarget {
  ...
  async listTools(
    ...
    await this.#queue.authorizeObservation({
      title: `${this.#host.serverName}：搜索工具`,
      description:
        `在 MCP 服务器 **${this.#host.serverName}** 的工具目录 ` +
        ...
        `(\`${this.#host.endpoint}\`) 中搜索 \`${plainInline(trimmed)}\`，` +
        `返回 ${tools.length} 个匹配项（最多 ${MAX_SEARCH_RESULTS} 个）。`,
    });
  ...
  async callTool(name: string, args?: Record<string, unknown>): Promise<McpCallResult> {
    ...
    return {
      ...
      actionId: staged.id,
      message:
        `在 ${host.serverName} 上调用“${name}”需要批准。如果这是在 agent 的 executeCode ` +
        `调用中，请立即从 executeCode 返回，以便批准请求出现在聊天中。批准后调用 ` +
        `getActionResult(${staged.id}) 获取结果。`,
    };
  }
