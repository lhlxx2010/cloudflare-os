...
export interface AuthenticatedApi extends RpcTarget {
  ...
  /**
   * Update an existing model owned by the current user. `id` and `profile.id` must remain equal so
   * existing chats and bindings keep referring to the same model; `config.model` is the
