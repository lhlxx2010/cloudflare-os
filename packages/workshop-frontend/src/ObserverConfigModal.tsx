  ...
  useEffect(() => {
    ...
    authenticatedApi
      ...
      .catch(err => {
        ...
        toasts.add({ title: '加载已连接账户失败', variant: 'error' })
      })
    ...
    <Dialog.Root open disablePointerDismissal onOpenChange={open => { if (!open) onCancel() }}>
      <Dialog className="p-6" size="lg">
        <Dialog.Title className="mb-2 text-lg font-semibold">
          {isRetry ? '再次验证你的访问权限' : '验证你的访问权限'}
        </Dialog.Title>
        <Text variant="secondary" size="sm" as="p">
          {isRetry
            ? '我们无法确认你能访问此工作区读取过的全部内容。请重新验证下方账户，或选择其他账户后重试。'
            : '打开此工作区前，请确认你自己的账户能够访问其中使用的已连接数据。'}
        </Text>
        ...
        {!ready || !vendorsReady ? (
          ...
          <div className="flex flex-col gap-4 mt-5">
            {needs.map(need => {
              ...
              const vendorName = vendor?.displayName || need.vendorId || '服务'
                ...
                <div key={need.gatekeeperId} className="rounded-xl border border-kumo-line bg-kumo-base p-4">
                  <div className={`flex items-center gap-3${matching.length === 0 && !need.failure ? '' : ' mb-3'}`}>
                    ...
                    {matching.length === 0 && vendor && (
                      ...
                      >
                        {connecting === need.vendorId ? '正在等待连接…' : '连接'}
                      </WorkshopButton>
                    )}
                  </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {matching.length === 1 ? (
                        <div className="flex min-h-10 items-center gap-3 rounded-lg border border-kumo-line bg-kumo-elevated/50 px-3 py-2">
                          <div className="min-w-0 flex-1">
                            <div className="text-[11px] leading-4 text-kumo-subtle">使用你的账户</div>
                          ...
                          {accountSatisfies(need, matching[0]) && (
                            <span className="flex shrink-0 items-center gap-1 text-xs font-medium text-kumo-success">
                              <CheckCircle size={15} weight="fill" /> 就绪
                            </span>
                          )}
                        </div>
                      ) : (
                        <Select
                          ...
                          placeholder={`选择一个 ${vendorName} 账户…`}
                          onValueChange={v =>
                      ...
                      {!vendor?.autoProvisionsAccount && (
                        ...
                        >
                          <Plus size={11} />
                          {connecting === need.vendorId ? '正在等待连接…' : '连接其他账户'}
                        </button>
                      )}
                    </div>
                  )}
