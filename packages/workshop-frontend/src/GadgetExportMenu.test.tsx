...
describe('GadgetExportMenu', () => {
  ...
  it('disables the export button and updates its tooltip while exporting', async () => {
    ...
    const trigger = container.querySelector<HTMLButtonElement>('[aria-label="导出应用"]')
    expect(trigger?.disabled).toBe(true)
    expect(trigger?.closest('[data-tooltip]')?.getAttribute('data-tooltip')).toBe('正在导出为 CSV')
    ...
    expect(trigger?.closest('[data-tooltip]')?.getAttribute('data-tooltip')).toBe('导出应用')
  })
  ...
  it('shows an empty state without hiding or disabling the export button', async () => {
    ...
    expect(container.textContent).toContain('此应用不支持导出。')
    expect(container.querySelector('[aria-label="导出应用"]')).not.toBeNull()
  })
  ...
  it('does not render the control without a selected Gadget', async () => {
    ...
    expect(container.querySelector('[aria-label="导出应用"]')).toBeNull()
  })
  ...
  it('loads fresh formats on every open and ignores a response after close', async () => {
    ...
    expect(container.querySelector('[role="status"][aria-label="正在加载导出格式"]')).not.toBeNull()
    await act(async () => { button('close export menu')?.click() })
    expect(container.querySelector('[role="status"][aria-label="正在加载导出格式"]')).toBeNull()
  ...
  it('shows an inline error and retries format discovery', async () => {
    ...
    try {
      ...
      expect(container.textContent).toContain('无法加载导出格式。')
