...
export default function GadgetEditor() {
  ...
  const {
    ...
    onInvalidShareKey: () => {
      toasts.add({ title: '共享链接无效或已过期。', variant: 'error' })
    },
  })
  ...
  const handleRenameWorkpiece = useCallback(async (workpieceId: WorkpieceId, title: string) => {
    ...
    try {
      ...
      toasts.add({ title: '重命名应用失败', variant: 'error' })
    } finally {
  ...
  const handleSaveTitle = async () => {
    ...
    } catch { toasts.add({ title: '更新标题失败', variant: 'error' }) }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-kumo-base">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-kumo-subtle">正在加载工作区…</p>
        </div>
        {observerConfig && (
          <ObserverConfigModal
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-kumo-base relative">
              ...
              {!layoutModeReady && (
                <div className="absolute inset-0 flex items-center justify-center bg-kumo-base">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-kumo-subtle">正在加载对话…</p>
                  </div>
                </div>
              )}
            </div>
        ...
        >
          ...
          >
            <div className="flex min-w-0 flex-1 items-center overflow-hidden">
              {paneShowsActivity ? (
                ...
                <PaneLabel
                  ...
                  badge={selectedGadgetSummary.chatId !== undefined ? '草稿' : undefined}
                />
              )}
            </div>
            ...
            <div className="flex flex-shrink-0 items-center gap-1.5">
              ...
              {!paneShowsActivity && (
                <GadgetExportMenu
                  gadget={selectedGadgetStub}
                  gadgetTitle={selectedGadgetSummary?.title ?? '应用'}
              ...
              {!paneShowsActivity && (
                <WorkshopIconButton
                  ...
                  title={activeTab === 'app' && !previewMode
                    ? '全屏'
                    : `全屏功能可在 ${formatOf(selectedGadgetSummary?.output).noun} 视图中使用`}
              ...
              <WorkshopIconButton
                aria-label={paneShowsActivity ? '关闭活动' : '关闭应用面板'}
                title="关闭"
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            ...
            <div
              ...
              aria-label={isGadgetFullscreen ? '应用全屏' : undefined}
              className={
              ...
              {isGadgetFullscreen && showFullscreenHint && (
                ...
                >
                  <div className="rounded-full border border-kumo-line bg-kumo-base/90 px-4 py-1.5 text-[13px] leading-[18px] text-kumo-default shadow-md backdrop-blur-sm">
                    按 <kbd className="rounded border border-kumo-line bg-kumo-elevated px-1.5 py-0.5 text-[11px] font-medium">Esc</kbd> 退出全屏
                  </div>
                </div>
              )}
            </div>
      ...
      <DeleteConfirmationDialog
        ...
        title="删除工作区？"
        description={<>这会移除 <span className="font-medium text-kumo-default">{metadata.title}</span>。此操作无法撤销。</>}
