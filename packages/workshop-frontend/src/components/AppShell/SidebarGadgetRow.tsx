...
export default function SidebarGadgetRow({
  ...
  return (
    <Link
      ...
      title={collapsed ? gadget.title || '未命名工作区' : undefined}
    >
      <div
      ...
      {!collapsed && (
        <>
          {renaming ? (
            ...
            <span className="min-w-0 flex-1 truncate">{gadget.title || '未命名工作区'}</span>
          )}
            ...
            <DropdownMenu>
              ...
              <DropdownMenu.Content className={MENU_CONTENT} style={MENU_POSITIONER_STYLE}>
                ...
                <DropdownMenu.Item
                  ...
                  className={MENU_ITEM}
                >
                  <ShareNetwork size={13} className="mr-2" /> 分享
                </DropdownMenu.Item>
      ...
      {collapsed && <span className="sr-only">{gadget.title || '未命名工作区'}</span>}
    </Link>
  )
}
