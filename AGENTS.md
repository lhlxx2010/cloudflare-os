...
- Type-aware oxlint rules are intentionally not enabled. The type-aware engine is tsgo (TypeScript 7), and three packages don't type-check under it: `workshop-frontend`, `integration-tests` and `workshop-backend` exceed tsgo's instantiation depth on capnweb's recursive `Stub`/`Stubify` types (TS2321/TS2589). The other 23 are clean (measured with `tsc` 7.0.2 over every tsconfig, `--noEmit`), so those three are the obstacle in the source — but nobody has run the type-aware rules themselves, so treat that as the known blocker rather than the only one. Among other things this means `no-floating-promises` is not enforced — which is just as well, since RPC promise pipelining (below) intentionally leaves promises unawaited. Type safety is still enforced by `tsc` through `pnpm types:check` and `pnpm build`.
- Two things also stand between here and TypeScript 7 generally, worth knowing before anyone bumps the catalog. TS 7's main export is `./lib/version.cjs` — the compiler API is gone from it — and two things here use that API: `scripts/build-gatekeeper-configurator.mjs` and `capnweb-validate`. The second is a trap: its peer range is `typescript: ">=5.7.0"`, which *admits* 7.x, so the install succeeds and the build breaks. Either would need an aliased 5.x (`"typescript-5": "npm:typescript@^5.9.3"`). And `typed-storage`, the only package still emitting declarations, needs an explicit `"rootDir": "./src"` under TS 7 (TS5011).
